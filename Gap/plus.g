...
local F,R,R1,Rels,S,U,V,varZ,e,f,p,phi;
  ...
  R:=[];
  if n > 3 then
    Add(R,Comm(varZ,U^(V^2))=1);
    Add(R,Comm(varZ,varZ^(V^2))=1);
  fi;
  if n > 4 then
    Add(R,Comm(varZ,V*U*U^V)=1);
  fi;
  Add(R,Comm(varZ,varZ^V)=1);
...
local F,R,R1,Rels,S,U,V,varZ,delta,e,f,p,phi;
  ...
  R1:=List(R1,r->phi(r));
  R:=[];
  Add(R,Comm(delta,U^V)=1);
  if n > 3 then
    Add(R,Comm(delta,V*U)=1);
  fi;
  Add(R,Comm(delta,delta^U)=1);
  Add(R,delta^(q-1)=1);
  Add(R,delta^varZ=delta^-1);
  Add(R,Comm(delta,varZ^V)=1);
  Rels:=Concatenation(List(R,r->LHS(r)*RHS(r)^-1),R1);
  return rec(val1:=F,
    val2:=Rels);
end;
...
local Delta,F,R1,R2,R3,R4,Rels,S,U,V,W,varZ,delta,e,f,n,p,phi,sigma,w;
  ...
  R3:=[];
  if e > 1 then
    Add(R3,Comm(sigma,delta*delta^U)=1);
    Add(R3,Comm(sigma,delta^(V^2))=1);
  fi;
  Add(R3,Comm(sigma,varZ*U)=1);
  if n > 3 then
    Add(R3,Comm(sigma,U^(V^2))=1);
  fi;
  if n > 4 then
    Add(R3,Comm(sigma,V*U*U^V)=1);
  fi;
  if n > 3 then
    Add(R3,Comm(sigma,varZ^(V^2))=1);
  fi;
  #   Steinberg relations
  R4:=[];
  ...
  Add(R4,Comm(sigma,sigma^(varZ^V))=1);
  if n=4 then
    Add(R4,Comm(sigma,sigma^(varZ^V*V^2))=1);
  fi;
...
local F,R,R1,Rels,S,U,V,varZ,e,f,p,phi,w;
  ...
  R1:=[];
  if n > 3 then
    Add(R1,Comm(varZ,U^(V^2))=1);
    Add(R1,Comm(varZ,varZ^(V^2))=1);
  fi;
  if n > 4 then
    Add(R1,Comm(varZ,V*U*U^V)=1);
  fi;
  Add(R1,varZ^2=1);
  Add(R1,Comm(varZ,(U^2)^V)=1);
  if n > 2 then
    Add(R1,varZ*varZ^V=varZ^(V*U));
  fi;
  Add(R1,Comm(varZ,U)=1);
  Add(R1,Comm(varZ,varZ^V)=1);
...
local Delta,F,I,R1,R2,R3,R4,Rels,S,U,V,W,varZ,b,e,f,n,p,phi,sigma,w;
  ...
  if n > 4 then
    Add(R3,Comm(sigma,V*U^-1*(U^-1)^V)=1);
  fi;
  if n > 3 then
    Add(R3,Comm(sigma,varZ^(V^2))=1);
  fi;
  if e > 1 then
    if n > 3 then
      Add(R3,Comm(sigma,lvarDelta^(V^2))=1);
    fi;
    Add(R3,Comm(sigma,lvarDelta^(varZ^V))=1);
    if n in Set([3,4]) then
      Add(R3,Comm(sigma,lvarDelta^(U^V)*lvarDelta^V)=1);
    fi;
  fi;
  Add(R3,Comm(sigma,varZ*U)=1);
  #   Steinberg relations
