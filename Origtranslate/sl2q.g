...
local B,varE,I,K,Projective,Q,Rels,U,c,delta,f,i,m,q,tau,tau1,w;
  ...
  Rels:=Concatenation(Rels,[tau^p=1,Tuple([tau,tau1])=1,Tuple([tau1,tau^delta])
   =1,(tau1*U*delta)^3=1]);
  if Projective then
  ...
  if IsSquare(1+w^1) then
    m:=Log(w^2,1+w^1);
    Append(TILDERels,tau^(delta^m)=tau*tau1);
    Append(TILDERels,tau1^(delta^m)=tau1*tau^delta);
  else
    m:=Log(w^2,(1+w^-1));
    Append(TILDERels,tau1^(delta^m)=tau*tau1);
    Append(TILDERels,tau^(delta^(m+1))=tau1*tau^delta);
  fi;
  ...
  for i in [2..e] do
    B[i+1]:=B[i-1]^delta;
  od;
  Append(TILDERels,Product(List([0..e],i->B[i+1]^c[i+1])));
...
local B,varE,I,K,Projective,Q,Rels,U,c,delta,f,i,m,q,r,tau,tau1,w;
  ...
  tau:=Q.2;
  U:=Q.3;
  Rels:=[Tuple([tau,tau^(delta^(QuoInt((q+1),4)))])=1,tau^(delta^m)
   =Tuple([tau^-1,delta^r])];
  if Projective then
    Rels:=Concatenation(Rels,[(tau*U)^3=1,(U*delta)
...
local B,varE,F,I,Rels,U,c,delta,f,i,m,q,tau,u,w;
  ...
  Rels:=[Product(List([0..e],i->B[i+1]^u[i+1])),(U*tau)^3,U^2,(U*delta)
   ^2,tau^2,(tau*delta)^(q-1),tau^(delta^m)*Tuple([tau,delta])^-1];
  return rec(val1:=F,
    val2:=Rels);
end;
