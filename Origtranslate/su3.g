#  File converted from Magma code -- requires editing and checking
#  Magma -> GAP converter, version 0.5, 11/5/18 by AH

#  Global Variables used: Append, BaseRing, BorelGenerators, BorelPresentation,
#  Characteristic, ChooseGamma, Coefficients, ComputeRight, ConstructTauMatrix,
#  ConstructVMatrix, DefineU0, Degree, DeltaMatrix, DiagonalMatrix, Eltseq,
#  Evaluate, ExceptionalCase, ExtendedGreatestCommonDivisor, FindSolutions,
#  FreeGroup, GF, GL, Gcd, Id, Identity, Integers, IsEven, IsOdd, IsPower,
#  IsUpperTriangular, Isqrt, LHS, Log, MatrixAlgebra, MatrixToTuple,
#  MinimalPolynomial, Normalise, Nrows, Parent, PolynomialRing,
#  PrimitiveElement, ProcessRelation, R2Relations, RHS, Relations, Reverse,
#  SLPForElement, SLPGroup, SU, SU32Generators, SU3Generators,
#  SpecialSLPForElement, TMatrix, TauMatrix, Trace, TupleToMatrix,
#  TwoPolynomials, Type, VMatrix, VectorSpace, Zero, phi

#  Defines: BorelGenerators, BorelPresentation, ChooseGamma, ComputeRight,
#  ConstructTauMatrix, ConstructVMatrix, DefineU0, DeltaMatrix,
#  ExceptionalCase, FindSolutions, MatrixToTuple, ProcessRelation, R2Relations,
#  SLPForElement, SU32Generators, SU32Presentation, SU3Generators,
#  SU3Presentation, SpecialSLPForElement, TMatrix, TauMatrix, TupleToMatrix,
#  TwoPolynomials, VMatrix

#   Explicit version of presentation for SU(3, q)
#   Last revised Ocotober 2018
#   to get paper version V (alpha, beta)
#   call V(alpha, beta - psi * alpha^(1+q))
VMatrix:=function(q,alpha,gamma)
local F,MA,beta,psi,v,w;
  F:=GF(q^2);
  w:=PrimitiveElement(F);
  if IsEvenInt(q) then
    psi:=Trace(w,GF(q))^(-1)*w;
    Assert(1,psi=1/(1+w^(q-1)));
  else
    psi:=(-1/2)*FORCEOne(F);
  fi;
  beta:=psi*alpha^(1+q)+gamma;
  MA:=MatrixAlgebra(F,3);
  v:=[1,alpha,beta,0,1,-alpha^q,0,0,1]*FORCEOne(MA);
  return v*FORCEOne(GL(3,F));
end;

DeltaMatrix:=function(q,alpha)
local delta;
  delta:=DiagonalMat([alpha,alpha^(q-1),alpha^-q]);
  return delta*FORCEOne(GL(3,q^2));
end;

TauMatrix:=function(q,gamma)
local F,MA,tau;
  F:=GF(q^2);
  MA:=MatrixAlgebra(F,3);
  tau:=[1,0,gamma,0,1,0,0,0,1]*FORCEOne(MA);
  return tau*FORCEOne(GL(3,F));
end;

TMatrix:=function(q)
local MA,t;
  MA:=MatrixAlgebra(GF(q^2),3);
  t:=Zero(MA);
  t[1][3]:=1;
  t[2][2]:=-1;
  t[3][1]:=1;
  return t;
end;

BorelGenerators:=function(q)
local F,alpha,beta,delta,tau,v,w;
  F:=GF(q^2);
  w:=PrimitiveElement(F);
  v:=VMatrix(q,1,0);
  beta:=v[1][3];
  alpha:=v[1][2];
  Assert(1,Trace(beta,GF(q))=-alpha^(q+1));
  if IsEvenInt(q) then
    tau:=TauMatrix(q,1);
  else
    tau:=TauMatrix(q,w^(QuoInt((q+1),2)));
  fi;
  delta:=DeltaMatrix(q,w);
  return [v,tau,delta];
end;

SU32Generators:=function()
local Delta,F,V,beta0,q,t,v,v1,w,w0;
  q:=2;
  F:=GF(q^2);
  w:=PrimitiveElement(F);
  w0:=w^(q+1);
  beta0:=w^(1+QuoInt((q^2+q),2));
  V:=VectorSpace(F,2);
  v:=VMatrix(q,1,0);
  v1:=VMatrix(q,w^2,0);
  lvarDelta:=DeltaMatrix(q,w);
  t:=TMatrix(q);
  return [v,v1,lvarDelta,t];
end;

SU3Generators:=function(q)
if q=2 then
    return SU32Generators();
  fi;
  return Concatenation(BorelGenerators(q),[TMatrix(q)]);
end;

#   g is upper-triangular matrix with just one non-zero entry in top right
#  corner;
#  write as word in Borel subgroup generators 
SpecialSLPForElement:=function(g,q,W)
local Delta,F,Gens,I,R,Tau,V,c,delta,entry,g,matrix,tau,theta,v,w,word,z;
  if g=g^0 then
    return rec(val1:=Identity(W),
      val2:=g);
  fi;
  Assert(1,g[1][1]=1 and g[1][2]=0 and g[2][3]=0);
  v:=W.1;
  tau:=W.2;
  delta:=W.3;
  F:=GF(q^2);
  w:=PrimitiveElement(F);
  Gens:=SU3Generators(q);
  V:=Gens[1];
  lvarTau:=Gens[2];
  lvarDelta:=Gens[3];
  entry:=lvarTau[1][3];
  I:=Integers();
  z:=g[1][3];
  if z<>0 then
    theta:=w^-(q+1);
    R:=SubStructure(F,theta);
    c:=Eltseq((entry^-1*z)*FORCEOne(R));
    c:=List(c,x->x*FORCEOne(I));
    word:=Product(List([1..Size(c)],i->(tau^(delta^(i-1)))^c[i]));
    matrix:=Product(List([1..Size(c)],i->(lvarTau^(lvarDelta^(i-1)))^c[i]));
    g:=matrix^-1*g;
    Assert(1,g[1][3]=0);
  fi;
  return rec(val1:=word,
    val2:=g);
end;

#   find solutions x and y to Lemma 4.1
FindSolutions:=function(q)
local F,c,d,found,m,n,psi,t,w,w0,x,y;
  F:=GF(q^2);
  w:=PrimitiveElement(F);
  n:=q+1;
  m:=q-2;
  w0:=w^(q+1);
  if IsEvenInt(q) then
    c:=w0;
    d:=1-w0;
    x:=-Log(c) mod (q^2-1);
    y:=-Log(d) mod (q^2-1);
    Assert(1,w^(x*m)+w^(y*m)=1 and w^(-x*n)+w^(-y*n)=1);
    Assert(1,Size(SubStructure(GF(q^2),w^(x*m)))=q and Size(SubStructure(GF(q^2)
     ,w^(x*n)))=q);
    return rec(val1:=x,
      val2:=y);
  fi;
  psi:=w^(QuoInt((q+1),2));
  for t in F do
    if t^2=psi^2 then
      continue;
    fi;
    c:=t*(t^2+3*psi^2)*(t+psi)*(t^2-psi^2)^-2;
    d:=(c-c^(q-1));
    if c=0 or d=0 then
      continue;
    fi;
    x:=-Log(c) mod (q^2-1);
    y:=-Log(d) mod (q^2-1);
    if w^(x*m)+w^(y*m)=1 and w^(-x*n)+w^(-y*n)=1 then
      found:=Size(SubStructure(GF(q^2),w^(x*m)))=q^2 and 
       Size(SubStructure(GF(q^2),w^(x*n)))=q;
      if found then
        return rec(val1:=x,
          val2:=y);
      fi;
    fi;
  od;
  Error("Failed to find x and y");
end;

#   find two polynomials g, h of degree Degree (F) - 1 which satisfy
#      w^(2*q - 4) = g(r) + w^(q - 2) * h(r)
#   where r  = w^(x * (q - 2))
TwoPolynomials:=function(varE,F,p,q,w,x)
local P,W,e,g,h,l,one,r,two;
  e:=Degree(F);
  P:=PolynomialRing(GF(p));
  # Implicit generator Assg from previous line.
  l:=P.1;
  r:=w^(x*(q-2));
  W:=SubStructure(varE,r);
  one:=w^((q+1)*(q-2));
  two:=(w^(2*q-4)-one)*w^-(q-2);
  if one in W and two in W then
    g:=Eltseq(one*FORCEOne(W))*FORCEOne(P);
    h:=Eltseq(two*FORCEOne(W))*FORCEOne(P);
    if w^(2*q-4)=Evaluate(g,r)+w^(q-2)*Evaluate(h,r) then
      return rec(val1:=g,
        val2:=h);
    fi;
  fi;
  #   possibly solution is wrong: if so, must search over q^2 elements
  Assert(1,pair:=ForAny(F,
    u->ForAny(F,
      z->z+w^(q-2)*u=w^(2*q-4))));
  one:=pair[1];
  two:=pair[2];
  g:=Eltseq(one*FORCEOne(W))*FORCEOne(P);
  h:=Eltseq(two*FORCEOne(W))*FORCEOne(P);
  Assert(1,w^(2*q-4)=Evaluate(g,r)+w^(q-2)*Evaluate(h,r));
  return rec(val1:=g,
    val2:=h);
end;

#   Presentation for Borel subgroup
#   power relation needed in Borel presentation only
#   if used to set up SU(3, q) then AddPower = false;
BorelPresentation:=function(q)
local 
   A,AddPower,B,Delta,varE,F,Gens,I,K,L,R,Rels,Tau,V,W,a,b,b1,b2,b3,b4,delta,e,
   f,lhs,m1,m2,m3,m4,matrix,p,rhs,tau,theta,v,w,w0,word,x,y;
  AddPower:=ValueOption("AddPower");
  if AddPower=fail then
    AddPower:=false;
  fi;
  W:=SLPGroup(3);
  v:=W.1;
  tau:=W.2;
  delta:=W.3;
  I:=Integers();
  varE:=GF(q^2);
  F:=GF(q);
  e:=Degree(F);
  p:=Characteristic(varE);
  f:=Degree(F);
  w:=PrimitiveElement(varE);
  w0:=w^(q+1);
  theta:=w^(q-2);
  Gens:=BorelGenerators(q);
  V:=Gens[1];
  lvarTau:=Gens[2];
  lvarDelta:=Gens[3];
  # =v= MULTIASSIGN =v=
  y:=FindSolutions(q);
  x:=y.val1;
  y:=y.val2;
  # =^= MULTIASSIGN =^=
  A:=(lvarDelta^1)^x;
  B:=(lvarDelta^1)^y;
  Rels:=[];
  #   R1
  a:=(delta^1)^x;
  b:=(delta^1)^y;
  #   this relation can be omitted for SU(3, q)
  if AddPower then
    Append(TILDERels,delta^(q^2-1)=1);
  fi;
  if IsEvenInt(q) then
    Append(TILDERels,v^2=tau);
  else
    Append(TILDERels,v^p=1);
  fi;
  Append(TILDERels,tau^p=1);
  #   R2
  Append(TILDERels,tau=tau^(a)*tau^(b));
  if IsOddInt(p) then
    Append(TILDERels,tau=tau^(b)*tau^(a));
  fi;
  if e > 1 then
    m1:=MinimalPolynomial(w0^-x);
    b1:=Coefficients(m1);
    b1:=List(b1,x->x*FORCEOne(I));
    Append(TILDERels,Product(List([1..Size(b1)],i->(tau^(a^(i-1)))^b1[i])=1));
  fi;
  if e=1 or Gcd(x,q^2-1) > 1 then
    K:=SubStructure(F,w0^-x);
    b2:=Eltseq((w0^-1)*FORCEOne(K));
    b2:=List(b2,x->x*FORCEOne(I));
    Append(TILDERels,Product(List([1..Size(b2)],i->(tau^(a^(i-1)))^b2[i])
     =tau^(delta)));
  fi;
  #   R3
  #   (i)
  lhs:=v;
  rhs:=v^a*v^b*SpecialSLPForElement((V^A*V^B)^-1*V,q,W);
  Append(TILDERels,lhs=rhs);
  if IsOddInt(p) then
    lhs:=v;
    rhs:=v^b*v^a*SpecialSLPForElement((V^B*V^A)^-1*V,q,W);
    Append(TILDERels,lhs=rhs);
  fi;
  #   (ii)
  Append(TILDERels,Tuple([v^a,tau])=1);
  Append(TILDERels,Tuple([v^b,tau])=1);
  #   (iii)
  lhs:=Tuple([v,v^a]);
  rhs:=SpecialSLPForElement(Tuple([V,V^A]),q,W);
  Append(TILDERels,lhs=rhs);
  #   (iv)
  if IsEvenInt(q) and e > 1 then
    lhs:=Tuple([v^delta,v^a]);
    rhs:=SpecialSLPForElement(Tuple([V^lvarDelta,V^A]),q,W);
    Append(TILDERels,lhs=rhs);
    lhs:=Tuple([v^delta,v^b]);
    rhs:=SpecialSLPForElement(Tuple([V^lvarDelta,V^B]),q,W);
    Append(TILDERels,lhs=rhs);
  fi;
  #   (v)
  m2:=MinimalPolynomial(w^(x*(q-2)));
  b:=Coefficients(m2);
  b:=List(b,x->x*FORCEOne(I));
  lhs:=Product(List([1..Size(b)],i->(v^(a^(i-1)))^b[i]));
  matrix:=Product(List([1..Size(b)],i->(V^(A^(i-1)))^b[i]));
  rhs:=SpecialSLPForElement(matrix,q,W);
  Append(TILDERels,lhs=rhs);
  #   (vi)
  if IsOddInt(q) and Gcd(x,q^2-1) > 1 then
    K:=SubStructure(varE,w^(x*(q-2)));
    b:=Eltseq((w^(q-2))*FORCEOne(K));
    b:=List(b,x->x*FORCEOne(I));
    L:=V^(lvarDelta);
    R:=Product(List([1..Size(b)],i->(V^(A^(i-1)))^b[i]));
    matrix:=R^-1*L;
    word:=SpecialSLPForElement(matrix,q,W);
    lhs:=v^delta;
    rhs:=Product(List([1..Size(b)],i->(v^(a^(i-1)))^b[i])*word);
    Append(TILDERels,lhs=rhs);
  elif IsEvenInt(q) then
    # =v= MULTIASSIGN =v=
    m4:=TwoPolynomials(varE,F,p,q,w,x);
    m3:=m4.val1;
    m4:=m4.val2;
    # =^= MULTIASSIGN =^=
    b3:=Coefficients(m3);
    b3:=List(b3,x->x*FORCEOne(I));
    b4:=Coefficients(m4);
    b4:=List(b4,x->x*FORCEOne(I));
    L:=V^(lvarDelta^2);
    R:=Product(List([1..Size(b3)],i->(V^(A^(i-1)))^b3[i])
     *Product(List([1..Size(b4)],i->(V^(lvarDelta*A^(i-1)))^b4[i])));
    matrix:=R^-1*L;
    word:=SpecialSLPForElement(matrix,q,W);
    lhs:=v^(delta^2);
    rhs:=Product(List([1..Size(b3)],i->(v^(a^(i-1)))^b3[i])
     *Product(List([1..Size(b4)],i->(v^(delta*a^(i-1)))^b4[i])*word));
    Append(TILDERels,lhs=rhs);
  fi;
  Rels:=List(Rels,r->LHS(r)*RHS(r)^-1);
  return rec(val1:=W,
    val2:=Rels);
end;

ChooseGamma:=function(q,beta,eta,w,zeta)
local gamma,t,x;
  Assert(1,Trace(eta,GF(q))<>0);
  Assert(1,Trace(beta,GF(q))<>0);
  t:=Trace(beta,GF(q))*Trace(eta,GF(q))^-1;
  gamma:=t*eta-beta;
  Assert(1,Trace(gamma,GF(q))=0);
  x:=(w*zeta^-1)*(beta+gamma);
  Assert(1,x in GF(q) and x<>0);
  return gamma;
end;

#   definition of U0
DefineU0:=function(q)
local Alpha,F,alpha,beta0,eta,gamma0,n,t,w,w0,zeta;
  F:=GF(q^2);
  w:=PrimitiveElement(F);
  w0:=w^(q+1);
  beta0:=w^(1+QuoInt((q^2+q),2));
  t:=Trace(beta0,GF(q));
  n:=Log(w0*FORCEOne(GF(q)),-t*FORCEOne(GF(q)));
  alpha:=w^n;
  Assert(1,alpha^(q+1)=-t);
  if q mod 3<>2 then
    lvarAlpha:=[alpha];
  else
    lvarAlpha:=[alpha,alpha*w^(q-1),alpha*w^(2*(q-1))];
    Assert(1,ForAll(lvarAlpha,a->a^(q+1)=-t) and IsPower(w^(q-1),3)=false and 
     IsPower(w^(2*(q-1)),3)=false);
  fi;
  zeta:=-w^(QuoInt((q^2+q),2));
  Assert(1,zeta^2=w0);
  eta:=w^-1*zeta;
  gamma0:=ChooseGamma(q,beta0,eta,w,zeta);
  return rec(val1:=lvarAlpha,
    val2:=gamma0);
end;

MatrixToTuple:=function(A)
local A,F,alpha,beta,psi,q,w;
  if Type(A)=AlgMatElt or Type(A)=GrpMatElt then
    A:=A[1];
  fi;
  F:=BaseRing(Parent(A));
  q:=RootInt(Size(F));
  w:=PrimitiveElement(F);
  alpha:=A[2];
  beta:=A[3];
  if IsEvenInt(q) then
    psi:=Trace(w,GF(q))^(-1)*w;
  else
    psi:=-1/2;
  fi;
  beta:=A[3]-psi*(alpha^(1+q));
  return [alpha,beta];
end;

TupleToMatrix:=function(q,v)
return VMatrix(q,v[1],v[2]);
end;

ComputeRight:=function(q,U)
local V,m,r,s,t;
  V:=Parent(U[1]);
  r:=Reversed(Eltseq(U[1]))*FORCEOne(V);
  s:=Normalise(r);
  s[2]:=-s[2];
  t:=MatrixToTuple(s);
  m:=TupleToMatrix(q,t);
  Assert(1,MatrixToTuple(m)=t);
  return rec(val1:=m,
    val2:=t);
end;

ProcessRelation:=function(G,v)
local U,d,left,lv,q,rest,right,rv,t;
  q:=RootInt(Size(BaseRing(G)));
  t:=TMatrix(q);
  U:=TupleToMatrix(q,v);
  # =v= MULTIASSIGN =v=
  rv:=ComputeRight(q,U);
  right:=rv.val1;
  rv:=rv.val2;
  # =^= MULTIASSIGN =^=
  rest:=t^-1*U*t*right^-1*t^-1;
  d:=DiagonalMat(BaseRing(G),List([1..Length(rest)],i->rest[i][i]));
  left:=rest*d^-1;
  Assert(1,IsUpperTriangular(left));
  lv:=MatrixToTuple(left);
  Assert(1,t^-1*U*t=left*d*t*right);
  return rec(val1:=lv,
    val2:=rv,
    val3:=left,
    val4:=d,
    val5:=t,
    val6:=right);
end;

#   construct v-matrix whose 1, 2 entry is w^power, as word
#  in delta = Delta (w) and v = VMatrix(1, 0),
#  where w is primitive element for GF(q^2) 
ConstructVMatrix:=function(q,delta,v,power)
local A,varE,F,I,m,n,nu,w,z;
  # =v= MULTIASSIGN =v=
  m:=ExtendedGreatestCommonDivisor(q-2,q^2-1);
  z:=m.val1;
  n:=m.val2;
  m:=m.val3;
  # =^= MULTIASSIGN =^=
  F:=GF(q^2);
  w:=PrimitiveElement(F);
  nu:=w^z;
  varE:=SubStructure(F,nu);
  A:=Eltseq(power*FORCEOne(varE));
  I:=Integers();
  A:=List(A,a->a*FORCEOne(I));
  return Product(List([0..Size(A)-1],i->(v^(delta^(n*i)))^A[i+1]));
end;

#   construct tau-matrix whose 1, 3 entry is power, as word in
#  tau = TauMatrix (a) where a = w^((q+ 1) div 2) or 1 and
#  delta = Delta (w), where w is primitive element for GF(q^2) 
ConstructTauMatrix:=function(q,delta,tau,power)
local A,varE,F,I,gamma,w,w0;
  F:=GF(q^2);
  I:=Integers();
  w:=PrimitiveElement(F);
  w0:=w^(q+1);
  varE:=SubStructure(F,w0);
  if IsOddInt(q) then
    gamma:=power/w^(QuoInt((q+1),2));
  else
    gamma:=power;
  fi;
  Assert(1,gamma in varE);
  A:=Eltseq(gamma*FORCEOne(varE));
  A:=List(A,a->a*FORCEOne(I));
  return Product(List([0..Size(A)-1],i->(tau^(delta^(-i)))^A[i+1]));
end;

#   write matrix mat as word in Borel subgroup generators delta, v, tau
SLPForElement:=function(q,delta,v,tau,mat_v,mat,wdelta,wv,wtau)
local A,B,a,b,m,wA,wB;
  a:=mat_v[1];
  if a<>0 then
    A:=ConstructVMatrix(q,delta,v,a);
    wA:=ConstructVMatrix(q,wdelta,wv,a);
  else
    A:=v^0;
    wA:=wv^0;
  fi;
  m:=A*mat^-1;
  b:=m[1][3];
  B:=ConstructTauMatrix(q,delta,tau,m[1][3]);
  wB:=ConstructTauMatrix(q,wdelta,wtau,m[1][3]);
  Assert(1,mat=A*B^-1);
  return rec(val1:=wA,
    val2:=wB^-1,
    val3:=A,
    val4:=B^-1);
end;

R2Relations:=function(q)
local 
   Alpha,F,G,L,R,U,V,W,a,a1,b,b1,beta0,d,delta,e,gamma0,i,left,lhs,lv,m,mats,
   pow,psi,rhs,right,rv,t,tau,tau0,u,v,w,w0,w1,wdelta,wlhs,wrhs,wt,wtau,wv,x,x1,
   y,y1,z,z1;
  F:=GF(q^2);
  w:=PrimitiveElement(F);
  w0:=w^(q+1);
  beta0:=w^(1+QuoInt((q^2+q),2));
  V:=VectorSpace(F,2);
  G:=SU(3,q);
  L:=SU3Generators(q);
  v:=L[1];
  tau:=L[2];
  delta:=L[3];
  t:=L[4];
  W:=SLPGroup(4);
  wv:=W.1;
  wtau:=W.2;
  wdelta:=W.3;
  wt:=W.4;
  # =v= MULTIASSIGN =v=
  gamma0:=DefineU0(q);
  lvarAlpha:=gamma0.val1;
  gamma0:=gamma0.val2;
  # =^= MULTIASSIGN =^=
  if IsEvenInt(q) then
    psi:=Trace(w,GF(q))^(-1)*w;
  else
    psi:=-1/2;
  fi;
  tau0:=VMatrix(q,0,gamma0);
  mats:=[];
  for i in [1..Size(lvarAlpha)] do
    e:=beta0-psi*lvarAlpha[i]^(q+1);
    Append(TILDEmats,VMatrix(q,lvarAlpha[i],e));
  od;
  mats:=Concatenation(mats,List(mats,m->m*tau0));
  Append(TILDEmats,tau0);
  U:=List( # {@-list:
    mats,m->MatrixToTuple(m));
  #   determine relation u^t = u_L * Delta^pow * t * u_r
  R:=[];
  for u in U do
    # =v= MULTIASSIGN =v=
    right:=ProcessRelation(G,u);
    lv:=right.val1;
    rv:=right.val2;
    left:=right.val3;
    d:=right.val4;
    t:=right.val5;
    right:=right.val6;
    # =^= MULTIASSIGN =^=
    pow:=Log(d[1][1]);
    m:=TupleToMatrix(q,u);
    # =v= MULTIASSIGN =v=
    b1:=SLPForElement(q,delta,v,tau,u,m,wdelta,wv,wtau);
    a:=b1.val1;
    b:=b1.val2;
    a1:=b1.val3;
    b1:=b1.val4;
    # =^= MULTIASSIGN =^=
    # =v= MULTIASSIGN =v=
    y1:=SLPForElement(q,delta,v,tau,lv,left,wdelta,wv,wtau);
    x:=y1.val1;
    y:=y1.val2;
    x1:=y1.val3;
    y1:=y1.val4;
    # =^= MULTIASSIGN =^=
    # =v= MULTIASSIGN =v=
    w1:=SLPForElement(q,delta,v,tau,rv,right,wdelta,wv,wtau);
    z:=w1.val1;
    w:=w1.val2;
    z1:=w1.val3;
    w1:=w1.val4;
    # =^= MULTIASSIGN =^=
    lhs:=t^-1*a1*b1*t;
    rhs:=(x1*y1)*delta^pow*t*z1*w1;
    Assert(1,lhs=rhs);
    wlhs:=wt^-1*a*b*wt;
    wrhs:=(x*y)*wdelta^pow*wt*z*w;
    Append(TILDER,wlhs=wrhs);
  od;
  return rec(val1:=W,
    val2:=R);
end;

#   presentation for SU(3, 2) on its standard generators 
SU32Presentation:=function()
local F,Projective,Q,R,Rels,W,phi;
  Projective:=ValueOption("Projective");
  if Projective=fail then
    Projective:=false;
  fi;
  F:=FreeGroup(7);
  Q:=F/[F.1^2/One(F),
    F.2/One(F),
    F.3^2/One(F),
    F.4^-1*F.3*F.4^-1/One(F),
    F.6^-1*F.3*F.6^-1/One(F),
    F.4^-1*F.6^-1*F.4*F.6^-1/One(F),
    F.5/One(F),
    (F.3*F.1)^3/One(F),
    F.1*F.4*F.1*F.4^-1*F.1*F.6^-1*F.1*F.6/One(F),
    F.1*F.6^-1*F.1*F.4^-1*F.1*F.4*F.6^-1*F.7/One(F)];
  W:=SLPGroup(7);
  R:=Relations(Q);
  Rels:=List(R,r->LHS(r)*RHS(r)^-1);
  phi:=GroupHomomorphismByImages(Q,W,
    GeneratorsOfGroup(Q),List([1..7],i->W.i));
  Rels:=List(Rels,r->phi(r));
  if Projective then
    Append(TILDERels,W.7^2);
  fi;
  return rec(val1:=W,
    val2:=Rels);
end;

#   presentation for SU(3, q) for q = 2, 3, 5 on presentation generators
ExceptionalCase:=function(q)
local F,Gamma,R,T,a,b,t,tau,v,v1;
  if q=2 then
    F:=SLPGroup(4);
    # Implicit generator Assg from previous line.
    v:=F.1;
    v1:=F.2;
    lvarGamma:=F.3;
    t:=F.4;
    a:=Tuple([v,t]);
    b:=(a^2)^v;
    T:=[a=Tuple([v,t]),b=(a^2)^v,a^3=1,b^3=1,lvarGamma^3=1,Tuple([a,lvarGamma])
     =1,Tuple([b,lvarGamma])=1,Tuple([a,b])
     =lvarGamma^-1,a^v=b^-1,b^v=a*lvarGamma^1,a^v1=a*b*a,b^v1=a*b*lvarGamma^1,
     v^2=v1^2,v1^2=Tuple([v,v1]),t=v^2*a^2*b];
  elif q=3 then
    F:=SLPGroup(4);
    # Implicit generator Assg from previous line.
    v:=F.1;
    tau:=F.2;
    lvarGamma:=F.3;
    t:=F.4;
    T:=[F.1^3=One(F),F.4^2=One(F)
     ,F.3^-1*F.1^-1*F.3^-1*F.2^-1*F.1*F.3^2*F.1^-1=One(F)
     ,F.3^-1*F.1*F.3^-1*F.2^-1*F.1^-1*F.3^2*F.1=One(F)
     ,F.2*F.4*F.3^-2*F.2*F.4*F.2*F.4=One(F)
     ,F.3*F.2^-1*F.1^-1*F.4*F.3*F.2^-1*F.1*F.3*F.4*F.1*F.2^-1*F.4=One(F)];
  elif q=5 then
    F:=SLPGroup(4);
    T:=[F.1^5=One(F),F.3^1*F.2^2*F.3^-1*F.2=One(F)
     ,F.3^2*F.1^2*F.3^-2*F.1^-1=One(F),F.3^5*F.4*F.3*F.4=One(F)
     ,F.3^-1*F.1*F.4*F.1^-2*F.4*F.1*F.3*F.4=One(F)
     ,F.3*F.1*F.3^-1*F.2^-1*F.1^-1*F.3*F.1^-1*F.3^-1*F.1=One(F)
     ,
     F.1^-1*F.4*F.2^-1*F.1^-1*F.4*F.3^-1*F.1^2*F.4*F.3*F.1^-1*F.4*F.3*F.2^-1=One\
(F)];

       fi;
  R:=List(T,x->LHS(x)*RHS(x)^-1);
  return rec(val1:=F,
    val2:=R);
end;

SU3Presentation:=function(q)
local Q,R,R1,R2,Rels,W,delta,phi,t,tau,v;
  if q in Set([2,3,5]) then
    # =v= MULTIASSIGN =v=
    R:=ExceptionalCase(q);
    Q:=R.val1;
    R:=R.val2;
    # =^= MULTIASSIGN =^=
    return rec(val1:=Q,
      val2:=R);
  fi;
  W:=SLPGroup(4);
  v:=W.1;
  tau:=W.2;
  delta:=W.3;
  t:=W.4;
  #   relations of type R1 
  # =v= MULTIASSIGN =v=
  R1:=BorelPresentation(q);
  Q:=R1.val1;
  R1:=R1.val2;
  # =^= MULTIASSIGN =^=
  phi:=GroupHomomorphismByImages(Q,W,
    GeneratorsOfGroup(Q),List([1..3],i->W.i));
  Rels:=List(R1,r->phi(r)=1);
  #   relations of type R2 
  # =v= MULTIASSIGN =v=
  R2:=R2Relations(q);
  Q:=R2.val1;
  R2:=R2.val2;
  # =^= MULTIASSIGN =^=
  phi:=GroupHomomorphismByImages(Q,W,
    GeneratorsOfGroup(Q),List([1..4],i->W.i));
  Rels:=Concatenation(Rels,List(R2,r->phi(r)));
  Append(TILDERels,t^2=1);
  Append(TILDERels,delta^t=delta^-q);
  Rels:=List(Rels,r->LHS(r)*RHS(r)^-1);
  return rec(val1:=W,
    val2:=Rels);
end;

#  
#  for q in [3..1000] do
#  if IsOdd(q) and  IsPrimePower (q) then
#  q;
#  Q, R := OddBorelPresentation (q);
#  X := SU3Generators (q);
#  S := Evaluate (R, X); assert #Set (S) eq 1;
#  end if;
#  end for;
#  
#  for q in [7..1000] do
#  if  IsPrimePower (q) then
#  q;
#  Q, R := BorelPresentation (q);
#  X := SU3Generators (q);
#  S := Evaluate (R, X); assert #Set (S) eq 1;
#  end if;
#  end for;
#  
#  for q in [2..1000] do
#  if IsPrimePower (q) then
#  q;
#  Q, R := BorelPresentation (q);
#  X := BorelGenerators (q);
#  S := Evaluate (R, X); assert #Set (S) eq 1;
#  end if;
#  end for;
#  
#  for q in [2..100000] do
#  if IsPrimePower (q) then
#  q;
#  time Q, R := SU3Presentation (q);
#  X := SU3Generators (q);
#  S := Evaluate (R, X); assert #Set (S) eq 1;
#  end if;
#  end for;
#  
#  for e in [2..20] do
#  e;
#  q := 2^e;
#  G, R := SU3Presentation (q);
#  X := SU3Generators (q);
#  S := Evaluate (R, X); assert #Set (S) eq 1;
#  end for;
#  
#  SetGlobalTCParameters (:Hard, CosetLimit:=10^8, Print:=10^5);
#  for q in [2..1000] do
#  if IsPrimePower (q) then
#  q;
#  Q, R := SU3Presentation (q);
#  X := SU3Generators (q);
#  S := Evaluate (R, X); assert #Set (S) eq 1;
#  F := SLPToFP (Q, R);
#  H := sub<F | F.1, F.2, F.3>;
#  I := CosetImage (F, H);
#  RandomSchreier (I);
#  "Comp factors", CompositionFactors (I);
#  end if;
#  end for;


