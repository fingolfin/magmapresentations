...
local F,R,R1,Rels,S,U,V,varZ,phi;
  ...
  R1:=[];
  if n > 2 then
    Append(TILDER1,Tuple([varZ,U^V])=1);
  fi;
  if n > 3 then
    Append(TILDER1,Tuple([varZ,V*U^-1])=1);
  fi;
  Append(TILDER1,varZ^2=1);
  Append(TILDER1,Tuple([varZ,U^2])=1);
  Append(TILDER1,Tuple([varZ,varZ^U])=1);
...
local Delta,F,OMIT,R,R1,Rels,S,U,V,varZ,phi;
  ...
  Append(TILDER1,lvarDelta^(varZ*varZ^U)=lvarDelta^-1);
  if n=2 then
    Append(TILDER1,Tuple([lvarDelta,lvarDelta^varZ])=1);
  fi;
  Rels:=Concatenation(List(R1,r->LHS(r)*RHS(r)^-1),R);
  return rec(val1:=F,
    val2:=Rels);
...
local 
  ...
  if IsPrimeInt(q) then
    ...
    b:=(1/w)*FORCEOne(I);
    w:=w*FORCEOne(I);
    Append(TILDER3,lvarDelta=(sigma^U)^(w-w^2)*sigma^(b)*(sigma^U)^((w-1))
     *sigma^-1);
  fi;
  if e=1 then
    # =v= MULTIASSIGN =v=
  ...
  R2:=List(R2,r->phi(r));
  if e > 1 then
    Append(TILDER3,Tuple([sigma,lvarDelta^(varZ^U)])=1);
  fi;
  #   centraliser of tau
  R5:=[];
  if n > 2 then
    Append(TILDER5,Tuple([tau,U^V])=1);
    if e > 1 then
      Append(TILDER5,Tuple([tau,lvarDelta^V])=1);
    fi;
  fi;
  if n > 3 then
    Append(TILDER5,Tuple([tau,V*U^-1])=1);
  fi;
  Append(TILDER5,Tuple([tau,U^2*varZ^U])=1);
  if e > 1 and n=2 then
    Append(TILDER5,Tuple([tau,lvarDelta*lvarDelta^varZ])=1);
  fi;
  R6:=[];
  # =v= MULTIASSIGN =v=
  ...
  if n > 2 then
    ...
    Append(TILDER4,Tuple([sigma,tau^(V^2)])=1);
  fi;
  if n > 3 then
    Append(TILDER4,Tuple([sigma,sigma^(V^2)])=1);
  fi;
  Append(TILDER4,Tuple([sigma,sigma^(varZ^U)])=1);
  Append(TILDER4,Tuple([tau,tau^U])=(sigma^(varZ^U))^2);
  Append(TILDER4,Tuple([sigma,tau])=1);
  Append(TILDER4,Tuple([sigma^varZ,tau])=sigma*tau^(varZ*U));
...
local 
  ...
  local U,V,W,varZ,delta,p,s,sigma,t,tau;
    ...
    sigma:=Tuple([tau^(varZ*U),tau^(QuoInt((p+1),2))]);
    return [Tuple([delta^V,U]),varZ,tau,sigma,U,V];
  end;
...
local Delta,U,V,W,varZ,delta,gens,p,s,sigma,t,tau;
  ...
  if q mod 4=3 then
    lvarDelta:=U^2*Tuple([delta^V,U])^(QuoInt((q+1),4));
  elif q=9 then
    gens:=[Tuple([delta^V,U]),s^V,U,tau,sigma];
...
local Delta,U,V,W,varZ,sigma,t,tau;
  ...
  return [varZ^(V^-1),t,Tuple([varZ,lvarDelta^-1])^(V^-1),V,U];
end);
