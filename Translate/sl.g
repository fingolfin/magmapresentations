...
local F,OMIT,R,Rels,S,U,V,delta,tau;
  ...
  Rels:=[];
  if d > 3 then
    Add(Rels,Comm(delta,U^(V^2))=1);
  fi;
  if d > 4 then
    Add(Rels,Comm(delta,V*U*U^V)=1);
  fi;
  Add(Rels,delta*delta^V=delta^(V*U));
  Add(Rels,Comm(delta,delta^V)=1);
  if d > 3 then
    Add(Rels,Comm(delta,delta^(V^2))=1);
  fi;
  OMIT:=true;
  if not OMIT then
...
local F,I,R1,R2,Rels,S,U,V,a,b,delta,e,f,p,phi,tau,w,wm1;
  ...
  if e=1 then
    ...
    b:=(w-1)*FORCEOne(I);
    wm1:=(w^-1)*FORCEOne(I);
    Add(Rels,delta=(tau^a)^U*tau^(wm1)*(tau^b)^U*tau^-1);
  fi;
  ...
  #   centraliser of tau
  if d > 3 then
    Add(Rels,Comm(tau,U^(V^2))=1);
  fi;
  if d > 4 then
    if e > 1 then
      Add(Rels,Comm(tau,V*U*U^V)=1);
    else
      Add(Rels,Comm(tau,V*U^-1*(U^-1)^V)=1);
    fi;
  fi;
  if e > 1 then
    Add(Rels,Comm(tau,delta*(delta^2)^V)=1);
    if d > 3 then
      Add(Rels,Comm(tau,delta^(V^2))=1);
    fi;
  fi;
  #   Steinberg relations
  Add(Rels,Comm(tau,tau^V)=tau^(U^V));
  Add(Rels,Comm(tau,tau^(U^V))=1);
  Add(Rels,Comm(tau,tau^(U*V))=1);
  if d > 3 then
    Add(Rels,Comm(tau,tau^(V^2))=1);
  fi;
