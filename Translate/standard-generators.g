...
local F,M,P,a,b,d,delta,i,n,p,t,v,w;
  ...
  delta:=delta*FORCEOne(P);
  if d > 4 then
    v:=(DirectSumMat(Identity(MatrixAlgebra(F,d-4)),SpAdditionalElement(F)))
     *FORCEOne(P);
  elif d=4 then
...
local EvenCase,I,J,M,d,gamma,phi,q;
  ...
  if EvenCase then
    I[1][3]:=1;
    I[4][2]:=-1;
    J:=DiagonalMat(F,[gamma,gamma^-q,gamma^-1,gamma^q]);
  else
    if IsEvenInt(q) then
    ...
    I:=[1,phi,1,0,1,0,0,-1,1]*FORCEOne(M);
    J:=DiagonalMat(F,[gamma,gamma^(-q),gamma^(q-1)]);
  fi;
  I:=I*FORCEOne(GL(d,F));
  J:=J*FORCEOne(GL(d,F));
  return rec(val1:=I,
