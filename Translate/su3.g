...
local varE,F,c,d,found,m,n,psi,t,w,w0,x,y;
  F:=GF(q^2);
  w:=PrimitiveElement(F);
  n:=q+1;
  m:=q-2;
  ...
  if IsEvenInt(q) then
    x:=-(q+1) mod (q^2-1);
    varE:=GF(q);
    c:=Log(w0*FORCEOne(varE),(1-w0)*FORCEOne(varE));
    y:=-c*(q+1) mod (q^2-1);
...
local 
  ...
  if e > 1 then
    ...
    Add(Rels,Product(List([1..Size(b1)],i->(tau^(a^(i-1)))^b1[i])=1));
  fi;
  ...
  #   (ii)
  Add(Rels,Comm(v^a,tau)=1);
  Add(Rels,Comm(v^b,tau)=1);
  #   (iii)
  lhs:=Comm(v,v^a);
...
local F,Gamma,R,T,a,b,t,tau,v,v1;
  if q=2 then
    ...
    a:=Comm(v,t);
    b:=(a^2)^v;
    T:=[a=Comm(v,t]),b=(a^2)^v,a^3=1,b^3=1,lvarGamma^3=1,Tuple([a,lvarGamma)
     =1,Comm(b,lvarGamma])=1,Tuple([a,b)
