...
intrinsic ClassicalStandardGenerators (type :: MonStgElt, d :: RngIntElt, F :: FldFin : SpecialGroup := false, PresentationGenerators := false)
-> []
{return the Leedham-Green and O'Brien standard generators for the quasisimple 
 ...
 string type is one of SL, Sp, SU, Omega, Omega+, Omega-
   ...
   if type eq "Omega" then require IsOdd (d) and IsOdd (#F): 
      "Dimension and field size must be odd"; end if;

   if type in {"Omega+", "Omega-"}  then require (IsEven (d) and d ge 4): 
      "Dimension must be even and at least 4"; end if;

   if PresentationGenerators eq true then
      return Internal_PresentationGenerators (type, d, #F);
   end if;
...
intrinsic ClassicalStandardGenerators (type :: MonStgElt, d :: RngIntElt, q :: RngIntElt : SpecialGroup := false, PresentationGenerators := false)
-> []
{return the Leedham-Green and O'Brien standard generators for the quasisimple 
 classical group of specified type in dimension d and defining field F; the 
 string type is one of SL, Sp, SU, Omega, Omega+, Omega-
   ...
   return ClassicalStandardGenerators(type, d, GF(q) : SpecialGroup := SpecialGroup,
      PresentationGenerators := PresentationGenerators);
end intrinsic;
